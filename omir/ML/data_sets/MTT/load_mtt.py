...
def load_mtt_features(mtt_path, clip_info, index, start=0, stop=None, jmp=1, verbose=False):
    ...
    features = [(load_mtt_features_for_clip(mtt_path, clip_info, clip_id, verbose))
                for clip_id in index.values[start:stop:jmp]]
    return pandas.concat(features)


def load_mtt_audio(mtt_path,
                   clip_info, index, start=0, stop=None, jmp=1, verbose=False):
    audio = [(load_mtt_audio_for_clip(mtt_path, clip_info, clip_id, verbose))
             for clip_id in index.values[start:stop:jmp]]
...
def load_mtt_features_for_clip(mtt_path, clip_info, clip_id, verbose=False):
    ...
    try:
        beat_ts = pandas.DataFrame([1.0 for beat in beats],
                                   index=[int(math.ceil(float(beat.tatum.string)/SAMPLE_DURATION)) for beat in beats],
                                   columns=["beat"])
    except Exception as e:
        print("warning: unable to load beats for " + mtt_path)
        print(str(e))
        pass
    ...
    try:
        section_ts = pandas.DataFrame([1.0 for section in sections],
                                      index=[int(math.ceil(float(section["start"])/SAMPLE_DURATION))
                                             for section in sections],
                                      columns=["section"])
    except Exception as e:
        print("warning: unable to load sections for " + mtt_path)
        print(str(e))
        pass

    section_ts = section_ts.reindex(pandas.Index(range(0, num_frames)), fill_value=0.0)

    tempo_ts = pandas.DataFrame([tempo for i in range(0, num_frames)],
                                pandas.Index(range(0, num_frames)),
                                columns=["tempo"])
...
def usage():
    print("python load_mttpy --path <dataset_path>  --start <start_index> --stop <stop_index> [-v]")
    print("    --path   the path to the dataset")
    print("    --start  the clip index to start with")
    print("    --stop   the clip index to end with")
