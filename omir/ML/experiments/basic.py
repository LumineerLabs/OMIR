...
import numpy as np
from omir.ml.data_sets.mtt.load_mtt import load_mtt
import sys
import getopt
...
def build_model(batch_size, output_size):
    data_dim = 256
    timesteps = 1
    # num_classes = 3
    model = Sequential()
...
def build_combined_model(batch_size):
    return build_model(batch_size, 4)


def build_BPM_model(batch_size):
    return build_model(batch_size, 1)


def build_beats_model(batch_size):
    return build_model(batch_size, 3)


def usage():
    print("python basic.py ")  # todo: arguments
...
def main(args):
    ...
    if not eval:
        ...
        if train_type == "bpm":
            ...
            model = build_BPM_model(batch_size)
        if train_type == "beats":
            model = build_beats_model(batch_size)
            if load:
    ...
    if eval:
        bpm_model = build_BPM_model(batch_size)
        beats_model = build_beats_model(batch_size)
        ...
        print("evaluating")
        bpm_predictions = bpm_model.predict(reshape(np.array(data[3].values)),
                                            batch_size=eval_sample[1],
                                            verbose=verbose)
        beats_predictions = beats_model.predict(reshape(np.array(data[3].values)),
                                                batch_size=eval_sample[1],
                                                verbose=verbose)
